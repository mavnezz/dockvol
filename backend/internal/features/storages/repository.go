...
import (
	...
	db "dockvol-backend/internal/storage"
)
...
func (r *StorageRepository) Save(storage *Storage) (*Storage, error) {
	...
	err := database.Transaction(func(tx *gorm.DB) error {
		isNewStorage := storage.ID == uuid.Nil
		if isNewStorage {
			storage.ID = uuid.New()
		}
		...
		if isNewStorage {
			if err := tx.Create(storage).
				Omit("LocalStorage", "S3Storage", "NASStorage", "AzureBlobStorage", "FTPStorage", "SFTPStorage", "RcloneStorage").
				Error; err != nil {
				return err
			}
		} else {
